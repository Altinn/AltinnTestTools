...
public class CertificatePfx : ICertificateService
{
    ...
    {
        if (apiTokenSigningCertificate != null) return await Task.FromResult(apiTokenSigningCertificate);

        var fullPath = Path.Combine(Environment.GetEnvironmentVariable("ApplicationRootPath") ?? string.Empty, ApiTokenSigningCertPfxPath);
        ...
        apiTokenSigningCertificate = X509CertificateLoader.LoadPkcs12FromFile(fullPath, ApiTokenSigningCertPassword,
    ...
    {
        if (consentTokenSigningCertificate != null) return await Task.FromResult(consentTokenSigningCertificate);

        var fullPath = Path.Combine(Environment.GetEnvironmentVariable("ApplicationRootPath") ?? string.Empty, ConsentTokenSigningCertPfxPath);
        ...
        consentTokenSigningCertificate = X509CertificateLoader.LoadPkcs12FromFile(fullPath, ConsentTokenSigningCertPassword,
            X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
