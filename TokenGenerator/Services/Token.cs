...
public class Token(IOptions<Settings> settings, ICertificateService certificateHelper, IIssuer issuer) : IToken
{
    ...
    private static string GetIssuer(HttpRequest req, string env)
    {
        ...
        var tld = (env.ToLowerInvariant().StartsWith("at") || env.ToLowerInvariant().StartsWith("yt01")) ? "cloud" : "no";
